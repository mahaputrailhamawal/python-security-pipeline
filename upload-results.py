...
def upload_results(host, api_key, scanner, result_file, engagement_id, lead_id, environment, verify=False): # set verify to False if ssl cert is self-signed
    ...
    # Make a request to API (with 5 minute timeout for large files)
    response = requests.post(IMPORT_SCAN_URL, headers=headers, files=files, data=json, verify=verify, timeout=300)
